...
import {
  ...
  RadialBarChart, RadialBar, CartesianGrid, ReferenceLine
} from "recharts";
import {
...
const DarkTip = ({ active, payload, label, currency }) => {
  ...
  return (
    <div style={{background:"#1a2233",border:`1px solid ${C.border2}`,borderRadius:8,
...
const KPI = ({ icon: Icon, label, value, sub, color, spark, trend, trendLabel }) => {
  ...
  return (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 20px 14px",
      display:"flex",flexDirection:"column",gap:0,position:"relative",overflow:"hidden",transition:"border-color 0.2s"}}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        {trend !== undefined && (
          <div style={{display:"flex",alignItems:"center",gap:3,fontSize:11,fontWeight:600,
            color: trendPos ? C.emerald : trendNeg ? C.rose : C.textMid}}>
...
const Badge = ({ label, color, bg }) => (
  <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:11,fontWeight:600,
    padding:"3px 9px",borderRadius:20,background:bg||`${color}18`,color:color||C.textMid}}>
...
const Uploader = ({ onData, count }) => {
  ...
  const process = useCallback(file => {
    ...
    r.onload  = e => {
      try {
        const wb  = XLSX.read(ext==="csv"?e.target.result:new Uint8Array(e.target.result),
          {type:ext==="csv"?"binary":"array"});
        const ws  = wb.Sheets[wb.SheetNames[0]];
        const rows= XLSX.utils.sheet_to_json(ws);
        const miss= REQ_COLS.filter(c=>!Object.keys(rows[0]||{}).includes(c));
        if (miss.length) { setMsg({err:true,t:`Missing: ${miss.join(", ")}`}); return; }
        const clean = rows.map(r2=>({...r2,
      ...
      } catch(e2){ setMsg({err:true,t:"Parse error: "+e2.message}); }
    };
  ...
  return (
    <div>
      <div onClick={()=>ref.current?.click()}
        ...
        style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",borderRadius:10,
          border:`1.5px dashed ${drag?C.cyan:C.border2}`,cursor:"pointer",
          background:drag?`${C.cyan}08`:"transparent",transition:"all 0.18s"}}>
      ...
      {msg && (
        <div style={{marginTop:6,fontSize:11,padding:"5px 12px",borderRadius:6,
          background:msg.err?`${C.rose}12`:`${C.emerald}10`,
          color:msg.err?C.rose:C.emerald,border:`1px solid ${msg.err?C.rose:C.emerald}28`}}>
          {msg.t}
        </div>
      )}
    </div>
...
const Overview = ({ data }) => {
  ...
  const hiringByMonth = useMemo(()=>{
    const m={};
    data.forEach(r=>{const y=r["Join Date"]?.slice(0,7);if(y)m[y]=(m[y]||0)+1;});
  ...
  const byDept = useMemo(()=>{
    const m={};
    data.forEach(r=>{m[r.Department]=(m[r.Department]||0)+1;});
    return Object.entries(m).map(([n,c])=>({n,c})).sort((a,b)=>b.c-a.c);
  },[data]);

  const byGender = useMemo(()=>{
    const m={};
    data.forEach(r=>{m[r.Gender]=(m[r.Gender]||0)+1;});
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:14}}>
        ...
        <Card title="Gender breakdown" subtitle="Diversity ratio">
          <ResponsiveContainer width="100%" height={230}>
            <PieChart>
              <Pie data={byGender} cx="50%" cy="46%" innerRadius={62} outerRadius={92}
                dataKey="v" nameKey="n" paddingAngle={4} strokeWidth={0}>
        ...
        <Card title="Hiring trend" subtitle="Monthly hires over time">
          <ResponsiveContainer width="100%" height={190}>
            <AreaChart data={hiringByMonth} margin={{top:4,right:4,left:-22,bottom:0}}>
              ...
              <Tooltip content={<DarkTip/>}/>
              <Area type="monotone" dataKey="v" name="Hires" stroke={C.cyan} strokeWidth={2}
                fill="url(#htg)" dot={{r:3,fill:C.cyan,strokeWidth:0}}/>
            </AreaChart>
          </ResponsiveContainer>
        </Card>
...
const Compensation = ({ data }) => {
  const avgByDept = useMemo(()=>{
    const m={},cnt={};
    data.forEach(r=>{m[r.Department]=(m[r.Department]||0)+r.Salary;cnt[r.Department]=(cnt[r.Department]||0)+1;});
    return Object.entries(m).map(([n,t])=>({n,avg:Math.round(t/cnt[n])})).sort((a,b)=>b.avg-a.avg);
  },[data]);

  const byDesig = useMemo(()=>{
    const m={},cnt={};
    data.forEach(r=>{m[r.Designation]=(m[r.Designation]||0)+r.Salary;cnt[r.Designation]=(cnt[r.Designation]||0)+1;});
  ...
  const salRange = useMemo(()=>{
    ...
    return labels.map((lbl,i)=>({
      lbl, count:salaries.filter(s=>s>=buckets[i]&&s<buckets[i+1]).length
    }));
  },[data]);
      ...
      <Card title="Top 5 earners" subtitle="Highest compensated employees">
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {top5.map((emp,i)=>{
            ...
            return (
              <div key={emp["Employee ID"]} style={{display:"flex",alignItems:"center",gap:12,
                ...
                <div style={{width:36,height:36,borderRadius:10,background:`${aColor}22`,border:`1.5px solid ${aColor}44`,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:aColor,flexShrink:0}}>
...
const Directory = ({ data }) => {
  ...
  const filtered = useMemo(()=>{
    let rows = data;
    if (q)           { const lq=q.toLowerCase(); rows=rows.filter(r=>r["Employee Name"].toLowerCase().includes(lq)||r["Employee ID"].toLowerCase().includes(lq)||r.Designation.toLowerCase().includes(lq)); }
  ...
  const toggleSort = k => { if(sortK===k)setSortD(d=>d==="asc"?"desc":"asc"); else{setSortK(k);setSortD("desc");} setPage(1); };
  const reset      = () => { setQ("");setDept("All");setGen("All");setStat("All");setDesig("All");setPage(1); };

  const selS = {background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,
    padding:"6px 10px",color:C.textMid,fontSize:12,outline:"none",cursor:"pointer"};

  const SortArrow = ({k}) => sortK===k
    ? (sortD==="asc"?<ChevronUp size={12} color={C.cyan}/>:<ChevronDown size={12} color={C.cyan}/>)
    : <ChevronDown size={12} style={{opacity:0.25}}/>;

  const TH = ({label, sortable, k, w}) => (
    <th onClick={sortable?()=>toggleSort(k):undefined}
      style={{padding:"10px 14px",textAlign:"left",fontSize:10,fontWeight:600,letterSpacing:"0.08em",
        textTransform:"uppercase",color:sortK===k?C.cyan:C.textMuted,
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        {[
          ...
          <div key={x.label} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",
            borderRadius:8,background:C.card,border:`1px solid ${C.border}`,flex:"1 1 auto"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,alignItems:"center"}}>
          <div style={{position:"relative",flex:"1 1 200px"}}>
            ...
            <input value={q} onChange={e=>{setQ(e.target.value);setPage(1);}}
              ...
              style={{...selS,paddingLeft:28,width:"100%",boxSizing:"border-box"}}/>
          </div>
          {[
          ...
          <button onClick={reset}
            style={{...selS,display:"flex",alignItems:"center",gap:5,color:C.textMuted,background:"transparent"}}>
            <RefreshCw size={12}/> Reset
          </button>
          <button onClick={()=>exportCSV(filtered)}
            style={{...selS,display:"flex",alignItems:"center",gap:5,color:C.cyan,background:`${C.cyan}10`,borderColor:`${C.cyan}30`}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"auto"}}>
            ...
            <tbody>
              {paged.length===0&&(
                <tr><td colSpan={8} style={{padding:"40px",textAlign:"center",color:C.textMuted,fontSize:13}}>
              ...
              {paged.map((row,i)=>{
                ...
                return (
                  <tr key={row["Employee ID"]}
                    style={{borderBottom:`1px solid ${C.border}`,background:i%2===0?"transparent":"rgba(10,14,24,0.5)",
                      transition:"background 0.12s"}}
                    onMouseEnter={e=>e.currentTarget.style.background=`${C.cyan}06`}
                    onMouseLeave={e=>e.currentTarget.style.background=i%2===0?"transparent":"rgba(10,14,24,0.5)"}>
                    <td style={{padding:"10px 14px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:9}}>
                        <div style={{width:30,height:30,borderRadius:8,background:`${aColor}22`,
                          border:`1.5px solid ${aColor}44`,display:"flex",alignItems:"center",
                          justifyContent:"center",fontSize:10,fontWeight:700,color:aColor,flexShrink:0}}>
                    ...
                    <td style={{padding:"10px 14px"}}>
                      <span style={{fontSize:11,fontWeight:600,padding:"3px 8px",borderRadius:5,
                        background:`${C.purple}18`,color:C.purple}}>{row.Department}</span>
                    </td>
                    ...
                    <td style={{padding:"10px 14px"}}>
                      <Badge label={row.Status} color={row.Status==="Active"?C.emerald:C.rose}/>
                    </td>
                  </tr>
                );
              })}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
          padding:"10px 16px",borderTop:`1px solid ${C.border}`,background:"#0a0e18"}}>
          <span style={{fontSize:11,color:C.textMuted}}>
            {filtered.length} records · showing {(page-1)*PER+1}–{Math.min(page*PER,filtered.length)}
          </span>
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
              style={{...selS,padding:"4px 8px",display:"flex",background:"transparent",opacity:page===1?0.3:1}}>
              <ChevronLeft size={14}/>
            </button>
            {[...Array(totalPages)].map((_,i)=>(
              <button key={i} onClick={()=>setPage(i+1)}
                style={{...selS,padding:"4px 10px",minWidth:30,background:page===i+1?C.cyan:"transparent",
                  color:page===i+1?"#0B0F19":C.textMuted,fontWeight:page===i+1?700:400,
                  borderColor:page===i+1?C.cyan:C.border}}>
                {i+1}
              </button>
            )).slice(Math.max(0,page-3),page+2)}
            <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
              style={{...selS,padding:"4px 8px",display:"flex",background:"transparent",opacity:page===totalPages?0.3:1}}>
...
export default function TalentPulse() {
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",fontSize:14}}>
      ...
      {/* ── Sidebar ─────────────────────────────────────── */}
      <aside style={{width:open?248:62,flexShrink:0,background:C.surface,
        ...
        <div style={{padding:"18px 14px 14px",borderBottom:`1px solid ${C.border}`,
          ...
          <div style={{width:34,height:34,borderRadius:9,background:`linear-gradient(135deg,${C.emerald},${C.cyan})`,
        ...
        <nav style={{flex:1,padding:"10px 8px",display:"flex",flexDirection:"column",gap:2}}>
          {open && <div style={{fontSize:10,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",
            color:C.textMuted,padding:"8px 8px 4px"}}>Navigation</div>}
          {NAV.map(n=>{
            const Ic = n.icon;
            ...
            return (
              <button key={n.id} onClick={()=>setPage(n.id)}
                ...
                style={{display:"flex",alignItems:"center",gap:10,padding:open?"9px 10px":"10px",
                  borderRadius:9,border:"none",cursor:"pointer",width:"100%",textAlign:"left",
                  background:active?`${C.cyan}14`:"transparent",
                  transition:"background 0.14s",justifyContent:open?"flex-start":"center"}}
                onMouseEnter={e=>{if(!active)e.currentTarget.style.background=`${C.border}60`;}}
                onMouseLeave={e=>{e.currentTarget.style.background=active?`${C.cyan}14`:"transparent";}}>
                <div style={{width:28,height:28,borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",
                  background:active?`${C.cyan}20`:"transparent",flexShrink:0}}>
        ...
        <div style={{padding:"10px 8px",borderTop:`1px solid ${C.border}`,flexShrink:0}}>
          <button onClick={()=>setOpen(v=>!v)}
            style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border}`,
              cursor:"pointer",background:"transparent",color:C.textMuted,width:"100%",justifyContent:open?"flex-start":"center",transition:"all 0.14s"}}
            onMouseEnter={e=>{e.currentTarget.style.background=`${C.border}50`;e.currentTarget.style.color=C.textMid;}}
      ...
      <main style={{flex:1,minWidth:0,display:"flex",flexDirection:"column"}}>
        ...
        {/* Top bar */}
        <header style={{padding:"14px 24px",borderBottom:`1px solid ${C.border}`,
          background:`${C.surface}ee`,backdropFilter:"blur(16px)",
          position:"sticky",top:0,zIndex:10,display:"flex",flexDirection:"column",gap:10}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16}}>
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginTop:3}}>
                ...
                <span style={{fontSize:11,color:C.textMuted}}>
                  {data.length} employees · Privacy-first · All data stays in your browser
                </span>
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4,padding:"5px 10px",borderRadius:7,
        ...
        {/* Footer */}
        <footer style={{padding:"10px 24px",borderTop:`1px solid ${C.border}`,display:"flex",
